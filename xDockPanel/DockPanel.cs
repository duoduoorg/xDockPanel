...
namespace xDockPanel
{
    ...
    [ToolboxBitmap(typeof(resfinder), "xDockPanel.DockPanel.bmp")]
    ...
    public partial class DockPanel : Panel
    {
        ...
        public DockPanel()
        {
            ...
            Controls.AddRange(new Control[]    {
                ...
                DockWindows[DockState.Left],
                DockWindows[DockState.Right],
                DockWindows[DockState.Top],
                DockWindows[DockState.Bottom]
                });
        ...
        public void UpdateDockWindowZOrder(DockStyle dockStyle, bool fullPanelEdge)
        {
            ...
            {
                if (fullPanelEdge)
                    DockWindows[DockState.Left].SendToBack();
                else
                    DockWindows[DockState.Left].BringToFront();
            }
            else if (dockStyle == DockStyle.Right)
            {
                if (fullPanelEdge)
                    DockWindows[DockState.Right].SendToBack();
                else
                    DockWindows[DockState.Right].BringToFront();
            }
            else if (dockStyle == DockStyle.Top)
            {
                if (fullPanelEdge)
                    DockWindows[DockState.Top].SendToBack();
                else
                    DockWindows[DockState.Top].BringToFront();
            }
            else if (dockStyle == DockStyle.Bottom)
            {
                if (fullPanelEdge)
                    DockWindows[DockState.Bottom].SendToBack();
                else
                    DockWindows[DockState.Bottom].BringToFront();
            }
        }
        ...
        private int GetDockWindowSize(DockState dockState)
        {
            if (dockState == DockState.Left || dockState == DockState.Right)
            {
                ...
                return dockState == DockState.Left ? dockLeftSize : dockRightSize;
            }
            else if (dockState == DockState.Top || dockState == DockState.Bottom)
            {
                ...
                return dockState == DockState.Top ? dockTopSize : dockBottomSize;
            }
            else
                return 0;
        }
        ...
        protected override void OnLayout(LayoutEventArgs levent)
        {
            ...
            DockWindows[DockState.Left].Width = GetDockWindowSize(DockState.Left);
            DockWindows[DockState.Right].Width = GetDockWindowSize(DockState.Right);
            DockWindows[DockState.Top].Height = GetDockWindowSize(DockState.Top);
            DockWindows[DockState.Bottom].Height = GetDockWindowSize(DockState.Bottom);
        ...
        private void CalculateDockPadding()
        {
            ...
            if (AutoHideStripControl.GetNumberOfPanes(DockState.AutoHideLeft) > 0)
                DockPadding.Left = height;
            if (AutoHideStripControl.GetNumberOfPanes(DockState.AutoHideRight) > 0)
                DockPadding.Right = height;
            if (AutoHideStripControl.GetNumberOfPanes(DockState.AutoHideTop) > 0)
                DockPadding.Top = height;
            if (AutoHideStripControl.GetNumberOfPanes(DockState.AutoHideBottom) > 0)
        ...
        internal Rectangle DocumentWindowBounds
        {
            get
            {
                ...
                if (DockWindows[DockState.Left].Visible)
                {
                    rectDocumentBounds.X += DockWindows[DockState.Left].Width;
                    rectDocumentBounds.Width -= DockWindows[DockState.Left].Width;
                }
                if (DockWindows[DockState.Right].Visible)
                    rectDocumentBounds.Width -= DockWindows[DockState.Right].Width;
                if (DockWindows[DockState.Top].Visible)
                {
                    rectDocumentBounds.Y += DockWindows[DockState.Top].Height;
                    rectDocumentBounds.Height -= DockWindows[DockState.Top].Height;
                }
                if (DockWindows[DockState.Bottom].Visible)
                    rectDocumentBounds.Height -= DockWindows[DockState.Bottom].Height;
